...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (isset($_POST['update'])) {
        ...
        $sql = mysqli_query($con, "DELETE FROM `Image_db` WHERE `posttitle`='$posttitle'");
        ...
        if (!empty(array_filter($_FILES['fileUpload']['name']))) {
            ...
            foreach ($_FILES['fileUpload']['name'] as $id => $val) {
                ...
                $fileName = htmlspecialchars(mysqli_real_escape_string($con, $_FILES["fileUpload"]["name"][$id]));
                $extensionimg = substr($fileName, strlen($fileName) - 4, strlen($fileName));
                //rename the image file

                $fileNamenewfile = md5($fileName) . $extension;

                // Code for move image into directory

                move_uploaded_file($_FILES["fileUpload"]["tmp_name"][$id], "postimages/" . $fileNamenewfile);
                $query3 = mysqli_query($con, "INSERT INTO `Image_db`( posttitle, postimage) VALUES ('$posttitle','$fileNamenewfile')");
            }
        }
        $query = mysqli_query($con, "update tblposts set PostTitle='$posttitle',CategoryId='$catid',PostDetails='$postdetails',PostUrl='$url',Is_Active='$status',YT_link='$YT_link' where id='$postid'");
                                    ...
                                    <form name="addpost" method="post">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-12">
                                                ...
                                                <div class="card-box">
                                                    <h4 class="m-b-30 m-t-0 header-title"><b>optional
                                                            Images</b></h4>
                                                    <div class="custom-file">
                                                        <input type="file" name="fileUpload[]" class="custom-file-input"
                                                            accept="image/*" id="chooseFile" multiple>
